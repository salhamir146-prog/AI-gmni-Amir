...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function handleGeminiGenerate(model, bubble) {
    ...
    const requestBody = {
      ...
      generationConfig: {
        temperature: state.params.temperature || 1.0,
  ...
  async function handleImagenGenerate(model, promptText, bubble) {
    ...
    if (images.length > 0) {
      let html = '<div class="generated-images-grid" style="display:flex;flex-wrap:wrap;gap:12px;">';
      images.forEach(imgBase64 => {
        html += `<div class="img-wrapper" style="position:relative;border-radius:12px;overflow:hidden;box-shadow:var(--shadow-soft);">
          <img src="data:image/png;base64,${imgBase64}" class="generated-img" alt="تصویر تولید شده" style="max-width:280px;width:100%;display:block;"/>
          <a href="data:image/png;base64,${imgBase64}" download="imagen-${Date.now()}.png" style="position:absolute;bottom:8px;right:8px;background:rgba(0,0,0,0.6);color:#fff;padding:6px 10px;border-radius:8px;text-decoration:none;font-size:12px;">
  ...
  function appendUserMessage(text, file) {
    ...
    if (file && file.mimeType.startsWith('image/')) {
      contentHtml += `<br><img src="data:${file.mimeType};base64,${file.data}" class="chat-attached-img" alt="تصویر پیوست" style="max-width:200px;border-radius:12px;margin-top:8px;"/>`;
    }
